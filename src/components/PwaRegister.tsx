...
export function PwaRegister() {
  useEffect(() => {
    ...
    const register = async () => {
      try {
        // Без reg.update() у самій реєстрації: браузер сам перевіряє скрипт
        // воркера на кожній навігації, а примусова перевірка на кожному
        // завантаженні сторінки раз за разом породжувала нового воркера й
        // живила петлю вище.
        const reg = await navigator.serviceWorker.register("/sw.js", { scope: "/" });

        // PWA, запущена зі значка на головному екрані iOS, — не навігація.
        // Система розморожує ту саму WKWebView, JS не перезапускається, HTML
        // повторно не запитується — тож штатна перевірка воркера «на кожній
        // навігації» просто не відбувається. Застосунок може місяцями сидіти
        // на воркері, з яким був інстальований. Тому додатково перевіряємо
        // сценарій самі, коли вкладку/застосунок реально показують на екрані:
        // resume з бекграунду на iOS PWA — це видима подія visibilitychange,
        // навіть якщо сторінку не перезавантажували.
        //
        // reg.update() сам собою нешкідливий: він лише звіряє sw.js байт-у-
        // байт, і install/claim/controllerchange запускаються, тільки якщо
        // файл реально змінився — throttle на reload вище це так само покриє.
